...
export default function ProofVisualizer() {
  const { p, stage, selectedStage, setSelectedStage, history, result, reason, start, reset } = useProofVisualizer()
  ...
  const docLinks: Record<Stage, string> = {
    ...
    storage: 'https://docs.payid.nawasena-labs.com/docs/examples/create-nft-rule',
    ...
    decision: 'https://docs.payid.nawasena-labs.com/docs/core-concepts/overview',
    sign: 'https://docs.payid.nawasena-labs.com/docs/api/sdk-reference',
    validate: 'https://docs.payid.nawasena-labs.com/docs/contracts/contracts',
    complete: 'https://docs.payid.nawasena-labs.com/docs/core-concepts/overview',
  }
            ...
            return (
              <motion.div
                ...
                className={`p-2 rounded-xl text-center space-y-1 transition-all cursor-pointer ${
                  isSelected ? 'bg-[#00D084]/20 border border-[#00D084]/30' : isActive ? 'bg-[#00D084]/10' : isDone ? 'bg-white/5' : 'opacity-40'
                }`}
              ...
              >
                {(() => {
                  ...
                  return SIcon ? <SIcon className="w-4 h-4 mx-auto" style={{ color: isSelected ? s.color : isActive ? s.color : isDone ? s.color : '#64748B' }} /> : null
                })()}
                <div className={`text-[10px] font-medium ${p.textMain}`}>{s.title.split(' ')[0]}</div>
                <div className="w-full h-0.5 rounded-full bg-current opacity-20" style={{ color: isSelected || isActive ? s.color : isDone ? s.color : '#64748B' }} />
              </motion.div>
            )
          })}
        </div>
        ...
        <div className="relative space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className={`text-sm font-semibold ${p.textMain}`}>Live Process Log</h3>
                <p className={`text-xs ${p.textMuted}`}>Real-time pipeline execution</p>
              </div>
            </div>
            {selectedStage !== 'idle' && (
              <a
          ...
          <div className={`p-3 rounded-xl font-mono text-xs space-y-1.5 ${p.dark ? 'bg-white/3' : 'bg-black/3'}`}>
            {selectedStage === 'idle' && (
              <div className="flex items-center gap-2">
                ...
                <span className={p.textMuted}>Ready to start proof pipeline</span>
              </div>
            )}
            ...
            {selectedStage === 'storage' && (
              <>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={p.textMuted}>Fetching rule blob from IPFS...</span>
                </div>
              </>
            )}
              ...
              <>
                ...
                <div className="mt-2 space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={p.textMuted}>tx.amount {'<='} 100 USDC</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-gray-500">R3:</span>
                    <span className={p.textMuted}>oracle.kycLevel {'>='} 1</span>
                  </div>
                </div>
                <div className="flex items-center gap-2 mt-2">
                  <span className="text-[#F59E0B] animate-pulse">⚡</span>
                  <span className={p.textMuted}>executeRule(context, rule)</span>
            ...
            {selectedStage === 'decision' && (
              <>
                ...
                <div className="mt-2 space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[#00D084]">✓</span>
                    <span className={p.textMuted}>R2: 20 USDC {'<='} 100 USDC</span>
                    <span className="text-[#00D084]">PASS</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[#00D084]">✓</span>
                ...
                <div className="flex items-center gap-2 mt-2">
                  <span className={result === 'allow' ? 'text-[#00D084]' : 'text-[#EF4444]'}>
                    {result === 'allow' ? '✓' : '✖'}
                  </span>
                  <span className={p.textMuted}>Decision</span>
                  <span className={result === 'allow' ? 'text-[#00D084]' : 'text-[#EF4444]'}>{result?.toUpperCase()}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-gray-500">→</span>
            ...
            {selectedStage === 'validate' && (
              <>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={p.textMuted}>PayIDVerifier.verifyDecision()</span>
            ...
            {selectedStage === 'complete' && (
              <>
                <div className="flex items-center gap-2">
                  <span className="text-[#00D084]">✓</span>
                  <span className={p.textMuted}>On-chain verification passed</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[#00D084]">✓</span>
