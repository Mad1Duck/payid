...
function RuleCardItem({ rule, p, contracts, refetchMyRules, refetchSub }: any) {
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: p.dark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)',
      }}
    >
      <div className="aspect-video bg-[#00D084]/5 relative overflow-hidden group">
        <RuleImage uri={rule.uri} className="w-full h-full object-cover" />
      ...
      <div className="p-3">
        ...
        {loadingMeta && !meta ? (
          <p className="text-[10px] text-amber-500 animate-pulse mb-2">
          ...
          meta?.attributes && (
            <div className="flex flex-wrap gap-1 mt-2 mb-2">
              {meta.attributes.map((attr: any, idx: number) => (
                <span
                  key={idx}
                  className="px-1.5 py-0.5 rounded bg-black/5 text-[9px] font-medium text-gray-500"
                >
                ...
                try {
                  ...
                  if (err.message?.includes('not owned')) {
                    toast.error('Failed to add NFT', {
                      description:
                        'MetaMask has not detected this NFT yet. Please wait a few seconds for the RPC to sync, then try again.',
                    })
                  } else {
                    toast.error('Failed to add NFT', {
      ...
      >
        <span
          className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold transition-all ${
            isActive || done ? 'bg-[#00D084] text-[#0B0F1A]' : 'bg-current/20'
          }`}
        >
          {n}
        </span>
            ...
            >
              ...
              {!sub?.isActive && (
                <button
                  onClick={() =>
                    subscribe(
                      (subPrice as bigint | undefined) ?? parseEther('0.001'),
                    )
                  }
          ...
          <div className="p-5 space-y-5">
                    ...
                    <div key={t.label} className="relative">
                      <button
                        ...
                        className={`w-full h-full flex flex-col items-start gap-1.5 rounded-xl border text-left transition-all ${
                          simpleMode
                            ? 'p-4'
                            : 'px-2 py-3 items-center text-center'
                        } ${
                          isSelected
                      ...
                      {isUnavailable && (
                        <>
                          ...
                          <div className="absolute top-2 right-2 flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-[#F59E0B]/15 border border-[#F59E0B]/35 select-none pointer-events-none z-20">
                ...
                {format !== 'nested' ? (
                  ...
                  >
                    <p className={`text-sm ${p.textMuted}`}>
                      Nested rules combine multiple rule groups with AND/OR
                      logic.
                    </p>
                    <p className={`text-[11px] ${p.textMuted}`}>
                      Use Simple or Multi for visual building, then edit the
                      JSON directly.
                    </p>
                  </div>
                )}
                ...
                {(() => {
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const val = e.target.value
                            updateCond(0, {
                              value: val ? (Number(val) * 1e8).toString() : '',
                            })
                          }}
                          placeholder="45"
                          className={`${inp} w-full`}
                        />
                        <p className={`text-[10px] ${p.textMuted} mt-1`}>
                          Any payment below this USD value will be blocked
                          (regardless of token).
                        </p>
                      </div>
                    )
                  ...
                  if (t.label === 'QR Payments Only') {
                    return (
                      ...
                      >
                        <p className={`text-sm ${p.textMain}`}>
                          This rule is set. Only QR code payments will be
                          allowed.
                        </p>
                      </div>
                    )
                    ...
                    return (
                      <div>
                        ...
                        <input
                          type="number"
                          value={
                            limit ? (Number(limit) / 1_000_000).toString() : ''
                          }
                          onChange={(e) => {
                            const val = e.target.value
                            updateCond(0, {
                        ...
                        />
                        <p className={`text-[10px] ${p.textMuted} mt-1`}>
                          Payments that would exceed this daily limit will be
                          blocked.
                        </p>
                      </div>
                    )
              ...
              <p className={`text-sm ${p.textMain}`}>
                {simpleMode && selectedTemplate
                  ? (() => {
                      const c = conds[0]
                      if (!c?.field)
                        return 'Pick a template above to get started'
            ...
            <div className="space-y-2">
              ...
              <PremiumButton
                ...
                isLoading={
                  deployStage === 'uploading' || deployStage === 'creating'
                }
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-amber-400/70 mt-0.5">
                      You've used all your rule slots. Subscribe to unlock up to
                      3 slots.
                    </p>
                  </div>
                </div>
                          ...
                          <div className="relative z-10">
                            ...
                            <div className="flex items-start justify-between mb-3">
                              ...
                              `}
                                ...
                                <div>
                                  <span
                                    className={`text-sm font-bold ${isSelected ? 'text-[#F59E0B]' : isActivated ? 'text-[#00D084]' : p.textMain}`}
                                  >
                            ...
                            {/* Rule hash preview */}
                            <div
                              className={`mt-3 pt-3 border-t ${p.cardBorder}`}
                            >
                              <div
                                className={`text-[9px] font-mono ${p.textMuted} truncate`}
                              >
