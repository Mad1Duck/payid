...
type PolicyScanningProps = {
  pipeline: { id: string; name: string; status: 'pending' | 'running' | 'done' }[]
  ruleEvaluations?: RuleEvaluation[]
  riskScore?: number
}

export default function PolicyScanning({ pipeline, ruleEvaluations = [], riskScore }: PolicyScanningProps) {
              ...
              >
                  ...
                  {i < pipeline.length - 1 && (
                    <div
                      className={`w-px h-4 mt-1 ${
                        step.status === 'done' ? 'bg-[#00D084]/30' : p.dark ? 'bg-white/4' : 'bg-black/4'
                      }`}
                    />
                  )}
                </div>
                <div
                  className={`flex-1 py-2 text-[13px] font-medium transition-colors ${
                    step.status === 'done' ? 'text-[#00D084]' : step.status === 'running' ? p.textMain : p.textMuted
                  }`}
                >
                  {step.name}
                </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {rule.message && (
                          <div className={`text-[11px] ${p.textMuted} mt-0.5`}>{rule.message}</div>
                        )}
                      </div>
                      <div
