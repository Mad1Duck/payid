...
import {
  useMyRules, useActiveCombinedRule, usePayIDContext, useSubscription, useSubscribe,
} from 'payid-react'
import { toast } from 'sonner'
import { Link } from '@tanstack/react-router'
import { CheckCircle2, Info, Loader2, Zap, Crown, Cpu, Plus, Clock, FlaskConical, Play, ShieldCheck, ShieldAlert } from 'lucide-react'
...
type RuleObj = { if?: Cond; conditions?: Cond[]; logic?: string; message?: string }

function resolveField(field: string, ctx: { amount: number; timestamp: number; receiver: string }): unknown {
  ...
  for (const t of transforms) {
    ...
    else if (t === 'day') { const d = new Date(Number(val) * 1000).getDay(); val = d === 0 ? 7 : d }
...
function evalCond(c: Cond, ctx: { amount: number; timestamp: number; receiver: string }): boolean {
  const lhs = resolveField(c.field, ctx)
  const n = Number(lhs)
  const v = c.value
  switch (c.op) {
    case '>=': return n >= Number(v)
    case '<=': return n <= Number(v)
    case '>':  return n > Number(v)
    case '<':  return n < Number(v)
    case '==': return lhs == v
    case '!=': return lhs != v
    case 'in': return Array.isArray(v) && (v as unknown[]).some(x => Number(x) === n || x === lhs)
    case 'not_in': return Array.isArray(v) && !(v as unknown[]).some(x => Number(x) === n || x === lhs)
    case 'between': { const [lo, hi] = v as [number, number]; return n >= lo && n <= hi }
    case 'not_between': { const [lo, hi] = v as [number, number]; return !(n >= lo && n <= hi) }
...
function evalRule(rule: RuleObj, ctx: { amount: number; timestamp: number; receiver: string }): { pass: boolean; reason: string } {
  if (rule.if) {
    const pass = evalCond(rule.if, ctx)
    return { pass, reason: pass ? `${rule.if.field} ${rule.if.op} ${JSON.stringify(rule.if.value)} ✓` : rule.message ?? `${rule.if.field} ${rule.if.op} ${JSON.stringify(rule.if.value)} ✗` }
  }
  if (rule.conditions?.length) {
    ...
    return { pass, reason: pass ? 'Conditions passed' : rule.message ?? 'Conditions failed' }
  }
...
function buildRuleSetHash(
  ...
  const ruleNFTs = Array(tokenIds.length).fill(ruleNFT) as `0x${string}`[]
  return keccak256(
...
export default function RulesConsolePage() {
  ...
  const [ruleDetails, setRuleDetails] = useState<Record<string, { if?: { field?: string; op?: string; value?: unknown }; conditions?: Array<{field:string;op:string;value:unknown}>; logic?: string; message?: string }>>({})
  useEffect(() => {
    async function load() {
      const imgs: Record<string, string> = {}
      const details: Record<string, { if?: { field?: string; op?: string; value?: unknown }; conditions?: Array<{field:string;op:string;value:unknown}>; logic?: string; message?: string }> = {}
      for (const r of myRules) {
        if (!r.uri) continue
        const url = r.uri.replace('ipfs://', 'https://gateway.pinata.cloud/ipfs/')
        try {
          ...
          if (meta.rule) {
            ...
            console.log(`[RulesConsole] parsed rule for rule_${r.ruleId}:`, ruleVal)
            details[`rule_${r.ruleId.toString()}`] = ruleVal
          } else {
            console.log(`[RulesConsole] NO meta.rule for rule_${r.ruleId}`)
          }
        } catch (e) { console.error(`[RulesConsole] fetch failed for rule_${r.ruleId}:`, e) }
      }
      setNftImages(imgs)
  ...
  const handleCartridgeDrop = (cartridgeId: string, slotId: string) => {
    const slot = slots.find(s => s.id === slotId)
    if (slot?.cartridge) { toast.error('Slot Occupied', { description: 'Remove the current cartridge first' }); return }
  ...
  const handleCartridgeClick = (cartridgeId: string) => {
    if (!highlightedSlot) {
      ...
      if (!empty) { toast.error('No Empty Slots', { description: 'Remove a cartridge first' }); return }
  ...
  const handleRegister = () => {
    ...
    const selRules = filled
      .map(s => allRules.find(r => `rule_${r.ruleId.toString()}` === s.cartridge!.id))
    ...
    const ruleSetHash = buildRuleSetHash(contracts.ruleItemERC721 as `0x${string}`, tokenIds, ver)
    ...
    if (direction === 'none') {
      writeContract({ address: contracts.combinedRuleStorage as `0x${string}`, abi: COMBINED_ABI, functionName: 'registerCombinedRule', args: [ruleSetHash, ruleNFTs, tokenIds, ver] })
    } else {
      writeContract({ address: contracts.combinedRuleStorage as `0x${string}`, abi: COMBINED_ABI, functionName: 'registerCombinedRuleForDirection', args: [ruleSetHash, direction === 'inbound' ? 0 : 1, ruleNFTs, tokenIds, ver] })
    }
  }
  ...
  const stepBadge = (n: number, label: string, done: boolean) => (
    <div key={n} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-medium ${
      done ? 'border-[#00D084]/40 bg-[#00D084]/10 text-[#00D084]' : `${p.cardBorder} ${p.textMuted}`
    }`}>
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="max-w-5xl space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-start justify-between gap-4">
          ...
          {isConnected && (
            <div className={`shrink-0 flex items-center gap-2 px-3 py-2 rounded-xl border ${p.cardBorder} text-xs ${p.textMuted}`}
              style={{ backgroundColor: p.cardBg }}>
              <Crown className="w-3.5 h-3.5 text-[#F59E0B]" />
              <span>{sub?.isActive ? `Pro · ${myRules.length}/${sub.maxSlots}` : 'Free tier'}</span>
            </div>
          )}
        </div>
        ...
        {isConnected && (
          <div className="flex flex-wrap gap-2">
            ...
            {stepBadge(3, 'Activate',    myRules.some(r => r.tokenId > 0n))}
        ...
        {isConnected && (
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              { label: 'Total Rules',   value: myRules.length.toString(),                        color: '#0EA5E9' },
              { label: 'Policy',        value: activeCombined?.hash ? 'Live' : 'Off',            color: activeCombined?.hash ? '#00D084' : '#F59E0B' },
            ].map(s => (
      ...
      {!isConnected && (
        <div className={`${card} p-6 flex items-center gap-3`} style={{ backgroundColor: p.cardBg }}>
          ...
          <p className={`text-sm ${p.textMuted}`}>Connect your wallet to load rules and manage your policy.</p>
        </div>
      )}
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6 items-start">
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <div className={`${card} p-4 w-full flex flex-col items-center gap-3`} style={{ backgroundColor: p.cardBg }}>
                <p className={`text-[10px] font-mono uppercase tracking-widest ${p.textMuted} self-start`}>
                ...
                />
                <p className={`text-[9px] font-mono uppercase tracking-wider ${p.textMuted} opacity-50`}>
              ...
              {isConnected && (
                <div className={`${card} p-4 w-full space-y-3`} style={{ backgroundColor: p.cardBg }}>
                  <div className="flex items-center gap-2">
                    <Crown className="w-4 h-4 text-[#F59E0B]" />
                    <p className={`text-xs font-semibold uppercase tracking-wide ${p.textMuted}`}>Additional Slots</p>
                    <span className={`ml-auto text-[10px] px-2 py-0.5 rounded-full font-semibold ${
                      sub?.isActive ? 'bg-[#00D084]/10 text-[#00D084]' : 'bg-amber-500/10 text-amber-400'
                    }`}>
                  ...
                  <div className="flex items-center justify-between">
                    <p className={`text-[11px] ${p.textMuted}`}>
                      {myRules.length} of {sub?.maxSlots ?? 1} slot{sub?.maxSlots !== 1 ? 's' : ''} used
                    </p>
                    {!sub?.isActive && (
                      <p className="text-[10px] text-amber-400">Free tier = 1 slot</p>
                    )}
                  </div>
                  ...
                  {!sub?.isActive ? (
                    ...
                    >
                      {subPending ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Confirm in wallet…</>
                        : subConfirming ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Waiting…</>
                        : <><Crown className="w-3.5 h-3.5" /> Unlock 3 Slots — ~0.001 ETH / 30d</>}
                    </button>
                  ) : (
                    <div className={`p-3 rounded-xl border ${p.cardBorder} space-y-1`}>
                      <div className="flex items-center justify-between">
                        <p className={`text-[11px] ${p.textMuted}`}>Subscription active</p>
                        <span className="text-[10px] text-[#00D084] font-medium">{sub.maxSlots} slots</span>
                      </div>
                      {sub.expiry ? (
                        <p className={`text-[10px] ${p.textMuted}`}>
                          Expires {new Date(Number(sub.expiry) * 1000).toLocaleDateString()}
                        </p>
                      ) : null}
                      <button
                        onClick={() => subscribe(parseEther('0.001'))}
                      ...
                      >
                        {subPending || subConfirming ? <Loader2 className="w-3 h-3 animate-spin" /> : <><Clock className="w-3 h-3" /> Renew Subscription</>}
                      </button>
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-4">
              ...
              {activeCombined?.hash ? (
                <div className={`${card} overflow-hidden border-[#00D084]/30`} style={{ backgroundColor: 'rgba(0,208,132,0.03)' }}>
                  <div className="px-4 py-3 border-b border-[#00D084]/15">
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="w-4 h-4 text-[#00D084]" />
                      <span className="text-sm font-semibold text-[#00D084]">Active Policy</span>
                      <span className="ml-auto text-[10px] px-2 py-0.5 rounded-full bg-[#00D084]/15 text-[#00D084] font-semibold">● Live</span>
                    </div>
                  </div>
                  <div className="p-4 space-y-3">
                    {/* Policy hash */}
                    <div>
                      <p className={`text-[10px] font-medium ${p.textMuted} mb-0.5`}>Policy hash</p>
                      <p className="text-[11px] font-mono text-[#00D084] truncate">{activeCombined.hash}</p>
                    </div>
                    {/* Direction */}
                    <div className="flex items-center gap-2">
                      <span className={`text-[10px] ${p.textMuted}`}>Direction:</span>
                      <span className="text-[11px] font-medium text-[#00D084]">
                        {activeCombined.direction === 0 ? 'Inbound' : activeCombined.direction === 1 ? 'Outbound' : 'Both'}
                      </span>
                    </div>
                    {/* Rules list */}
                    <div className="space-y-1.5">
                      <p className={`text-[10px] font-medium ${p.textMuted}`}>
                        {activeCombined.ruleRefs.length} rule{activeCombined.ruleRefs.length !== 1 ? 's' : ''} enforced:
                      </p>
                      {activeCombined.ruleRefs.map((ref, idx) => {
                        const rule = myRules.find(r => r.tokenId === ref.tokenId)
                        return (
                          <div key={idx} className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${p.cardBorder}`} style={{ backgroundColor: p.dark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)' }}>
                            ...
                            <div className="flex-1 min-w-0">
                              {rule ? (
                                <>
                                  <p className={`text-[11px] font-medium ${p.textMain}`}>Rule #{rule.ruleId.toString()}</p>
                                  <p className="text-[10px] font-mono text-[#00D084]/70 truncate">{rule.ruleHash.slice(0, 20)}…</p>
                                </>
                              ) : (
                                <p className={`text-[11px] font-mono ${p.textMuted}`}>Token #{ref.tokenId.toString()}</p>
                              )}
                            </div>
                            {rule?.active && (
                              <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#00D084]/10 text-[#00D084] font-medium shrink-0">Active</span>
                            )}
                          </div>
                        )
                      })}
                    </div>
                    <p className={`text-[10px] ${p.textMuted}`}>v{activeCombined.version.toString()}</p>
                  </div>
                </div>
              ) : (
                <div className={`${card} p-4`} style={{ backgroundColor: p.cardBg }}>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm font-semibold ${p.textMain}`}>No Active Policy</span>
                  </div>
                  <p className={`text-[11px] ${p.textMuted} mt-1`}>Load rules into the console and register to go live.</p>
                </div>
              )}

              {/* Loaded slots summary */}
              <div className={`${card} p-4 space-y-3`} style={{ backgroundColor: p.cardBg }}>
                <p className={`text-[11px] font-semibold uppercase tracking-wide ${p.textMuted}`}>
                  Loaded Slots
                  {selectedSlots.length > 0 && (
                    <span className="ml-1 text-[#8B5CF6]">({selectedSlots.length}/3)</span>
                  )}
                </p>
                <div className="space-y-2">
                  {slots.map(slot => (
                    ...
                    >
                      <span className={`text-[9px] font-mono font-bold uppercase tracking-wider w-10 shrink-0 ${
                        slot.cartridge ? 'text-[#8B5CF6]' : p.textMuted
                      }`}>{slot.label}</span>
                      {slot.cartridge ? (
                        <div className="flex-1 min-w-0">
                          <p className={`text-[12px] font-medium ${p.textMain} truncate`}>{slot.cartridge.name}</p>
                          ...
                          {(() => {
                            ...
                            console.log(`[RulesConsole] render slot ${slot.label}, cartridge.id=${slot.cartridge.id}, detail=`, d, 'ruleDetails keys=', Object.keys(ruleDetails))
                            if (!d) return null
                            ...
                            const cond = simple?.field
                              ...
                              : d.conditions && d.conditions.length > 0
                                ? d.conditions.map(c => `${c.field} ${c.op} ${JSON.stringify(c.value)}`).join(` ${d.logic || 'AND'} `)
                                : d.message || null
                            console.log(`[RulesConsole] cond for ${slot.label}:`, cond)
                            return cond ? (
                              <p className="text-[10px] font-mono text-[#8B5CF6] truncate mt-0.5">{cond}</p>
                            ) : null
                          })()}
                        </div>
                      ) : (
                        <p className={`text-[11px] ${p.textMuted} italic`}>empty</p>
                      )}
                      {slot.cartridge && (
                ...
                >
                  <div className="flex items-center gap-3 px-5 py-4 border-b"
                    style={{
                      ...
                      background: p.dark ? 'rgba(139,92,246,0.06)' : 'rgba(139,92,246,0.04)',
                    }}>
                    <span className="w-7 h-7 rounded-full bg-[#8B5CF6] text-white font-bold text-sm flex items-center justify-center shrink-0">4</span>
                    <div className="flex-1">
                      <p className={`text-sm font-semibold ${p.textMain}`}>Register Policy</p>
                      <p className={`text-[11px] ${p.textMuted}`}>{selectedSlots.length} rule(s) selected</p>
                    </div>
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    {/* ── Demo / Test Rules ── */}
                    <div className={`rounded-xl border ${p.cardBorder} overflow-hidden`}>
                      <button
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-xs font-semibold ${p.textMain}`}>Test Rules Before Registering</span>
                        </div>
                        <span className={`text-[10px] ${p.textMuted}`}>{showDemo ? 'Hide' : 'Show'}</span>
                      </button>

                      {showDemo && (
                        <div className="px-4 pb-4 space-y-3 border-t"
                          style={{ borderColor: p.dark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)' }}>
                          <p className={`text-[11px] ${p.textMuted} pt-3`}>
                            Simulate a transaction to preview how your selected rules would evaluate.
                          </p>

                          {/* Selected rules summary */}
                          <div className="space-y-1.5">
                            {selectedSlots.map(s => (
                              <div key={s.id} className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px] border ${p.cardBorder}`}>
                                <span className="w-1.5 h-1.5 rounded-full bg-[#8B5CF6] shrink-0" />
                                <span className={`font-medium ${p.textMain}`}>{s.cartridge!.name}</span>
                                <span className={`${p.textMuted} ml-auto`}>{s.cartridge!.summary}</span>
                              </div>
                            ))}
                          </div>
                          ...
                          <div className="flex gap-2">
                            <div className="flex-1">
                              <label className={`block text-[10px] font-medium ${p.textMuted} mb-1`}>Amount (wei)</label>
                              <input
                                type="text"
                                value={demoAmount}
                            ...
                            <div className="flex-2">
                              <label className={`block text-[10px] font-medium ${p.textMuted} mb-1`}>Receiver</label>
                              <input
                                type="text"
                                value={demoReceiver}
                                onChange={e => setDemoReceiver(e.target.value)}
                                className={`w-full ${inp}`}
                                placeholder="0x..."
                              />
                            </div>
                          ...
                          <button
                            onClick={() => {
                              ...
                              setTimeout(() => {
                                ...
                                const ctx = { amount: amt, timestamp: Math.floor(Date.now() / 1000), receiver: demoReceiver }
                                let rejected = false
                                let rejectReason = ''
                                for (const s of selectedSlots) {
                                  const d = ruleDetails[s.cartridge!.id] as RuleObj | undefined
                                  if (!d) continue
                                  const { pass, reason } = evalRule(d, ctx)
                                  if (!pass) { rejected = true; rejectReason = reason; break }
                                }
                                if (rejected) {
                                  ...
                                  setDemoResult('ALLOW')
                                  setDemoReason('All selected rules passed (simulated)')
                                }
                              }, 300)
                            }}
                          ...
                          >
                            {demoResult === 'running' ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Evaluating…</>
                          ...
                          {demoResult === 'ALLOW' && (
                            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#00D084]/10 border border-[#00D084]/30">
                              ...
                              <div>
                                <p className="text-xs font-semibold text-[#00D084]">ALLOW</p>
                                <p className="text-[10px] text-[#00D084]/80">{demoReason}</p>
                              </div>
                            </div>
                          )}
                          {demoResult === 'REJECT' && (
                            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#EF4444]/10 border border-[#EF4444]/30">
                              ...
                              <div>
                                <p className="text-xs font-semibold text-[#EF4444]">REJECT</p>
                                <p className="text-[10px] text-[#EF4444]/80">{demoReason}</p>
                              </div>
                            </div>
                          )}
                        </div>
                    ...
                    {registerStage !== 'idle' && (
                      <p className={`text-xs text-center font-medium ${
                        registerStage === 'done'  ? 'text-[#00D084]' :
                        registerStage === 'error' ? 'text-[#EF4444]' : p.textMuted
                      }`}>
                        {registerStage === 'registering' && 'Registering on-chain…'}
                    ...
                    {myRules.map(r => (
                      <div key={r.ruleId.toString()}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className={`text-[10px] font-mono ${p.textMuted} truncate`}>{r.ruleHash.slice(0, 28)}…</p>
                        </div>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium shrink-0 ${
                          r.tokenId > 0n ? 'bg-[#00D084]/10 text-[#00D084]' : 'bg-amber-500/10 text-amber-400'
                        }`}>{r.tokenId > 0n ? '✓ Active' : 'Not activated'}</span>
                      </div>
                    ))}
                  </div>
                </div>
