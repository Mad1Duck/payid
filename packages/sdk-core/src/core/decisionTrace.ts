...
function traceRule(context: any, rule: AnyRule): RuleTraceEntry[] {
  ...
  if ("conditions" in rule) {
    return rule.conditions.map((cond: RuleCondition) => traceCondition(context, rule.id, cond));
  }

  // Format C: nested rules
  if ("rules" in rule) {
    return rule.rules.flatMap((child: AnyRule) => traceRule(context, child));
  }
...
export function buildDecisionTrace(
  ...
  return ruleConfig.rules.flatMap((rule: AnyRule) => traceRule(context, rule));
}